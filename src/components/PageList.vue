<template>
    <ul class="pagination ml-auto m-0" v-if="getLast() > 1">
        <li class="page-item" v-if="prevPageActive()">
            <a
                class="page-link"
                href="javascript:;"
                @click="choosePage(prevPage())"
            >
                <span>&laquo;</span>
            </a>
        </li>
...
export default {
    ...
    methods: {
        getCurrent() {
            ...
            try {
                current =
                    "current_page" in this.dataPage
                        ? this.dataPage.current_page
                        : this.current_page;
            } catch (error) {
                //console.log(error)
            }
        ...
        getLast() {
            ...
            try {
                last =
                    "last_page" in this.dataPage
                        ? this.dataPage.last_page
                        : this.last_page;
            } catch (error) {
                //console.log(error)
            }
